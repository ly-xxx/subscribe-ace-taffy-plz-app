...
if (-not $javaHome) {
    throw "Java 17 not found. Install Microsoft.OpenJDK.17 or update scripts/build-release-aab.ps1."
}
...
try {
    ...
    try {
        & .\gradlew.bat bundleRelease "-PreactNativeArchitectures=$($env:NAIWA_ABI_FILTERS)"
    }
    finally {
        Pop-Location
    }
...
if (Test-Path $aabPath) {
    ...
    throw "Build completed but release AAB was not found at: $aabPath"
}
